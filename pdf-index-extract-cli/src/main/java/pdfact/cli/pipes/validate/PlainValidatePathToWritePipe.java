...
public class PlainValidatePathToWritePipe implements ValidatePathToWritePipe {
     ...
     * @throws PdfActValidateException If the given path is not valid.
     */
        ...
        if (Files.exists(this.path)) {
            ...
            if (!Files.isRegularFile(this.path)) {
                throw new PdfActValidateException(
                        "The file '" + this.path + "' already exists, but is no regular file.");
            }

            // Make sure that the existing visualization file is writable.
            if (!Files.isWritable(this.path)) {
                throw new PdfActValidateException("The file '" + this.path + "' already exists, but isn't writable.");
            }
        }
    }
