...
public class PdfActUtils {
    ...
    public static Map<String, InputStream> readDirectory(String path) throws IOException {
        ...
        /**
         * // We need to distinguish, if we are in a jar file or not. CodeSource
         * codeSource = domain.getCodeSource(); Path jarFile =
         * Paths.get(codeSource.getLocation().getPath());
         *
         *
         * // Check, if we are in jar file. if (Files.isRegularFile(jarFile)) { try
         * (JarFile jar = new JarFile(jarFile.toFile())) { // Fetch all files in the
         * jar. final Enumeration<JarEntry> entries = jar.entries();
         *
         * while (entries.hasMoreElements()) { final String name =
         * entries.nextElement().getName(); // filter according to the path
         *
         * if (name.startsWith(path) && !name.equals(path)) { streams.put(name,
         * classLoader.getResourceAsStream(name)); } } } } else { // We are not in a jar
         * file. File directory = null; try { // Read the directory. directory = new
         * File(classLoader.getResource(path).toURI()); } catch (Exception e) { //
         * Nothing to do. }
         **/
        // Own code
        // Idea to catch an exception caused by an invalid character of the Path (":")
        // when calling outside of a jar and
        // the file is on an other drive than the java executable
        // May be a Windows Problem
