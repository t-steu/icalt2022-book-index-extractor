...
public class CaptionModule implements PdfTextSemanticizerModule {
    ...
    static {
        ...
        Pattern figureCaptionPattern = Pattern.compile("^(fig(\\.?|ure)|abbildung)\\s*\\d+", Pattern.CASE_INSENSITIVE);
        CAPTION_PATTERNS.put(SemanticRole.FIGURE, figureCaptionPattern);

        Pattern tableCaptionPattern = Pattern.compile("^(table|tabelle)\\s*\\d+(\\.|:)", Pattern.CASE_INSENSITIVE);
