...
public class XYCutTokenizeToTextAreasPipe extends XYCut implements TokenizeToTextAreasPipe {
    ...
    protected void tokenizeToTextAreas(Document pdf) throws PdfActException {
        ...
        Stream<Page> pageStream = pages.parallelStream().filter(page -> page != null);
        Stream<ElementList<TextArea>> textAreaStream = pageStream.map(page -> mapPage(pdf, page));

        List<ElementList<TextArea>> textAreasOfPages = textAreaStream.collect(Collectors.toList());

        this.numProcessedPages = textAreasOfPages.size();

        for (ElementList<TextArea> textAreas : textAreasOfPages) {
    ...
    private ElementList<TextArea> mapPage(Document pdf, Page page) {
     ...
     * @throws PdfActException If something went wrong while tokenization.
     */
    protected ElementList<TextArea> tokenizeToTextAreas(Document pdf, Page page) {
        ElementList<TextArea> result = new ElementList<>();

        ElementList<Character> characters = page.getCharacters();
        List<ElementList<Character>> areaCharsList = cutTextareasFast(pdf, page, characters);
     ...
     *
     * @param pdf  The PDF document to which the given text area belongs to.
     * @param page The PDF page to which the given text area belongs to.
     * @param a    The text area to process.
     ...
     * @param pdf  The PDF document to which the given text area belongs to.
     * @param page The PDF page to which the given text area belongs to.
     * @param area The text area to process.
     * @return The computed statistic.
     */
    protected CharacterStatistic computeCharacterStatistic(Document pdf, Page page, TextArea area) {
    ...
    public float assessVerticalCut(Document pdf, Page page, List<ElementList<Character>> halves) {
        ...
        // Don't allow the lane, if the resulting areas are too low. This should prevent
        // to split
        // single text lines into multiple areas, for example the page headers in
        // PDF0008.
    ...
    /**
     * Checks if there is a character in the first given list of characters with an
     * extraction order number i and a character in the second given list of
     * characters with extraction order number i + 1, where both characters overlap
     * vertically.
     *
     * @param left       The characters in the left half.
     * @param leftStats  The statistics about the characters in the left half.
     * @param right      The characters in the right half.
     * @param rightStats The statistics about the characters in the right half.
     * @return True if there is such a character pair, false otherwise.
     */
    protected boolean separatesConsecutiveCharacters(ElementList<Character> left, CharacterStatistic leftStats,
                                                     ElementList<Character> right, CharacterStatistic rightStats) {
