...
public class XYCutTokenizeToWordsPipe extends XYCut implements TokenizeToWordsPipe {
    ...
    protected void tokenizeToWords(Document pdf) throws PdfActException {
        ...
        List<List<ElementList<Word>>> wordLists = pdf.getPages().parallelStream()
                .map((page) -> this.transformToWords(pdf, page)).collect(Collectors.toList());

        for (List<ElementList<Word>> wordList : wordLists) {
            this.numProcessedTextLines++;
            this.numTokenizedWords += wordList.size();
        }
    }

    private List<ElementList<Word>> transformToWords(Document pdf, Page page) {
        List<ElementList<Word>> wordLists = new ArrayList<>();
     ...
     * @param page The PDF document to which the given page belongs to.
     * @param line The text line to process.
     * @return The words.
     * @throws PdfActException If something went wrong while tokenization.
     */
    public ElementList<Word> tokenizeToWords(Document pdf, Page page, TextLine line) {
        ElementList<Word> result = new ElementList<>();

        ElementList<Character> characters = line.getCharacters();
        List<ElementList<Character>> charLists = cutWordsFast(pdf, page, characters);
        Word word = null;
     ...
     *
     * @param page The PDF page in which the word is located.
     * @param word The word to process.
