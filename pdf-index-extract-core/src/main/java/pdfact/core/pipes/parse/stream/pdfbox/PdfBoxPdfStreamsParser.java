...
import pdfact.core.pipes.parse.stream.pdfbox.operators.color.*;
import pdfact.core.pipes.parse.stream.pdfbox.operators.graphic.*;
import pdfact.core.pipes.parse.stream.pdfbox.operators.text.*;
...
public class PdfBoxPdfStreamsParser implements PdfStreamsParser {
     ...
     */
    public void processType3Stream(Document pdf, Page page, PDType3CharProc proc, Matrix trm) throws IOException {
     ...
     */
    protected void processStreamOperators(Document pdf, Page page, PDContentStream stream) throws IOException {
     ...
     */
    public void processOperator(Document pdf, Page page, String operation, List<COSBase> arguments) throws IOException {
     ...
     * @throws IOException If there is an error processing the operation.
     */
    protected void processOperator(Document pdf, Page page, Operator op, List<COSBase> args) throws IOException {
    ...
    public Point2D.Double transformedPoint(double x, double y) {
        double[] position = {x, y};
        getGraphicsState().getCurrentTransformationMatrix().createAffineTransform().transform(position, 0, position, 0,
                1);
