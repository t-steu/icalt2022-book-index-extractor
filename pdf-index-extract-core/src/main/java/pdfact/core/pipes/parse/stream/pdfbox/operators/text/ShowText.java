...
public class ShowText extends OperatorProcessor {
    ...
    public void showGlyph(Document pdf, Page page, String glyph, int code, PDFont pdFont, Matrix trm)
        ...
        // See
        // https://stackoverflow.com/questions/48010235/pdf-specification-get-font-size-in-points
        // for an explanation why we can't use
        // engine.getGraphicsState().getTextState().getFontSize().
     ...
     * @param trm  The current text rendering matrix
     * @return The bounding box of the glyph or null, if the bounding box could not
     * be computed.
     * @throws IOException if something went wrong on computing the bounding box.
     */
    protected Rectangle computeGlyphBoundingBox(Document pdf, Page page, int code, PDFont font, Matrix trm)
            throws IOException {
     ...
     * @throws IOException if something went wrong on computing the bounding box.
     */
    protected Rectangle computeType3GlyphBoundingBox(Document pdf, Page page, int code, PDFont font, Matrix trm)
            throws IOException {
     ...
     * @throws IOException if something went wrong on computing the bounding box.
     */
    protected Rectangle computeNonType3GlyphBoundingBox(int code, PDFont font, Matrix trm) throws IOException {
        if (font == null) {
            return null;
        }
     ...
     */
    protected Rectangle transformBoundingBox(float minX, float minY, float maxX, float maxY, PDFont font, Matrix trm) {
        if (font == null) {
            return null;
        }
     ...
     * is computing an approximate bounding box, without respecting ascenders (like
     * "l") or descenders (like "g").
     *
     ...
     */
    protected Rectangle computePdfBoxGlyphBoundingBox(int code, PDFont font, Matrix trm) throws IOException {
        PDGraphicsState state = this.engine.getGraphicsState();
        Matrix ctm = state.getCurrentTransformationMatrix();
    ...
    /**
     * Returns true if the given character should be ignored on further processings;
     * false otherwise.
     */
    protected boolean ignoreCharacter(Character character) {
        ...
        // Trim the text. But don't use Java's trim() method here, because it doesn't
        // remove
        // non-breaking characters (Unicode \u00a0). PDF0006 contains such characters.
        // Instead, remove all leading and trailing "horizontal whitespaces", see for
        // example:
